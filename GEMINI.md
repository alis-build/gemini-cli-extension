...
- **Routers — discover a skill before acting.** For an ambiguous *functional* request — the
  ...
  - **Do NOT inspect, write, or edit code, run Define / Build / Deploy, or make commits before
    a skill is loaded.** If no skill fits, say so and offer `RequestSkill` (the current
    implementation emails the Alis Build team for review).
...
- **The CLI is self-documenting — consult it, don't memorise it.** This guide names only the
  ...
  `alis <cmd> --help`. Treat that output as the source of truth; this guide and the skills
